/**
 * 数据处理器:把原始活动数据(RawActivity)转换并写入 activities + splits。
 * 移植自源仓 src/lib/sync/processor.ts —— 核心逻辑逐字保留,仅两处富化改为 env 门控:
 * - 赛事匹配(race-matcher)依赖 Playwright,standalone 不移植 → ENRICH_RACE_MATCH 门控的 no-op。
 * - 天气(open-meteo,keyless 已移植)→ ENRICH_WEATHER 门控,默认关,避免每次同步打外部。
 */
import { and, eq } from 'drizzle-orm'

import { getDb } from '@/lib/db/client'
import { activities, splits } from '@/lib/db/schema'
import { calculateDistance, calculatePace, generateId } from '@/lib/utils'
...
/** 富化开关:形如 '1'/'true'/'yes'/'on' 视为开启,默认关。 */
function enrichEnabled(key: 'ENRICH_RACE_MATCH' | 'ENRICH_WEATHER'): boolean {
...
export async function syncActivity(rawActivity: RawActivity): Promise<string> {
  try {
    ...
    // 获取天气数据(仅室外活动,且 ENRICH_WEATHER 开启)。
    // Reason: open-meteo keyless 已移植,但默认关以免每次同步都打外部 API。
    let weatherData: string | null = null
    if (enrichEnabled('ENRICH_WEATHER') && !rawActivity.isIndoor) {
      try {
...
/**
 * 为缺少天气数据的室外活动批量获取天气(手动运维工具,显式调用)。
 *
