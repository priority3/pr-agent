/**
 * GPX 解析与轨迹几何工具。
 * 移植自源仓 src/lib/sync/parser.ts —— 逻辑逐字保留,仅:
 * - calculateDistance 复用 @/lib/utils(不在此重复定义)。
 * - 内联源仓 race-matcher 的 extractCoordinatesFromGPX(race-matcher 依赖 Playwright,standalone 不移植)。
 */
...
/**
 * 从 GPX XML 提取首个坐标点(经纬度)。
 * Reason: 内联自源仓 race-matcher.ts —— 天气富化需要活动起点坐标,
 * 但 race-matcher 依赖 Playwright 未移植,故把这个纯正则小工具收进 parser。
 */
