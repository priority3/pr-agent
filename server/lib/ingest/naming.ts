 ...
 *
 * 移植自源仓 src/lib/activity/naming.ts —— 供 Strava 适配器为跑步活动生成友好标题。
 * 差异:源仓依赖的 `@/types/activity` Activity 类型在此内联为最小结构,去除跨模块依赖。
 *
...
/** generateSmartName 所需的活动子集(内联替代源仓 Activity 类型)。 */
interface NamingActivity {
  distance: number
  startTime: Date
  gpxData: string | null
 ...
 */
export function generateSmartName(activity: NamingActivity, originalName: string): string {
