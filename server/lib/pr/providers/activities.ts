...
function activityLine(act: RecentActivityContext) {
  ...
  const weather = act.weather
    ...
    : ''
  const race = act.raceName ? `，赛事「${act.raceName}」` : ''
  return `- ${act.date}（${daysAgoLabel(act.daysAgo)}）：${TYPE_LABEL[act.type] ?? act.type} ${act.distanceKm} km，用时 ${act.durationMin} 分钟${act.paceText ? `，配速 ${act.paceText}/km` : ''}${act.avgHeartRate ? `，平均心率 ${act.avgHeartRate}` : ''}${race}${climb}${weather}`
}
...
export const activitiesProvider: ContextProvider = {
  ...
  tools: [
    {
      ...
      description:
        '查用户的真实运动记录(手表同步),返回含赛事名称、当次实测天气、爬升、起点相对位置(有轨迹/回填才有)。上下文里只有最近几条快照;当用户问"上次跑步/更早的记录/某段时间跑了什么/比赛记录"时用它。时间段用 after + before,比赛记录传 raceOnly=true,复盘具体某一天传 date。返回按时间倒序。',
      inputSchema: {
        type: 'object',
        properties: {
          type: {
          ...
          after: {
            type: 'string',
            description: 'YYYY-MM-DD,只看这一天(含)之后的记录,与 before 配合查询月份或训练周期',
          },
          date: {
            type: 'string',
            description: 'YYYY-MM-DD,只看这一天的活动——复盘某天的运动就传它(返回带那次的实测天气/爬升)',
          },
          raceOnly: { type: 'boolean', description: '只返回同步源明确标记了赛事名称的记录' },
          limit: { type: 'number', description: '返回条数,默认 5,最多 100；查月份或比赛记录时可传 100' },
        },
      },
    },
  ],
  executeTool: async (_name, rawInput) => {
    ...
    const rows = await queryActivityContext({
      ...
      raceOnly: input.raceOnly === true,
      limit: typeof input.limit === 'number' ? input.limit : undefined,
    })
    // 常跑地点整次调用只解析一次(有 TTL 缓存,但没必要每行都 await)
    const home = await getHomeLocation().catch(() => null)
    const longest = rows.reduce<RecentActivityContext | null>((current, row) => !current || row.distanceKm > current.distanceKm ? row : current, null)
    return JSON.stringify({
      count: rows.length,
      note: rows.length === 0 ? '没有匹配的记录(库里确实没有,可如实告知)' : undefined,
      summary: rows.length
        ? {
            totalDistanceKm: Number(rows.reduce((sum, row) => sum + row.distanceKm, 0).toFixed(2)),
